...
namespace Com.OneSignal.Editor.IMGUI
{
    ...
    class HyperToolbar
    {
        ...
        [SerializeField]
        List<HyperLabel> m_Buttons = new List<HyperLabel>();

        public List<HyperLabel> Buttons => m_Buttons;
        public int SelectionIndex => (from button in m_Buttons where button.IsSelectionLock select m_Buttons.IndexOf(button)).FirstOrDefault();
