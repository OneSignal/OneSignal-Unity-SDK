...
abstract class OneSignalBaseSettingsWindow<TWindow> : EditorWindow where TWindow : EditorWindow
{
    ...
    protected void OnLayoutGUI()
    {
        ...
        DrawScrollView(() => { OnTabsGUI(tabIndex); });
    }
    ...
    protected void DrawScrollView(Action onContent)
    {
        ...
        using (new GuiBeginScrollView(ref m_ScrollPos, GUILayout.Width(position.width),
            GUILayout.Height(position.height - m_HeaderHeight)))
        {
            onContent.Invoke();
            GUILayout.Space(1);
            if (Event.current.type == EventType.Repaint)
