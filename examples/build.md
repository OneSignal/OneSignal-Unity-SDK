...
- Top bar: centered title with OneSignal logo SVG/PNG + "Unity" text
...
App bar layout and styling are defined in styles.md.
...
- At runtime in HomeScreenController, set Screen.fullScreen = false and call
  ...
  Window.setStatusBarColor() via AndroidJavaObject using the primary color from styles.md
...
- Button styling: use the IAM button style from styles.md (primary color, white text,
  left-aligned with icon, full width, uppercase text)
...
- Full-screen semi-transparent overlay with centered spinner (use loading overlay color from styles.md)
...
Create OneSignal theme in Assets/UI/Theme/Theme.uss.
...
- Override Unity's internal `.unity-base-text-field__input` on `.input-field`
  to remove the default background and border (set background-color: transparent,
  border-width: 0), and move padding to the inner element so only the outer
  border shows
...
- Android status bar color is set programmatically via ConfigureAndroidStatusBar()
  ...
  Window.clearFlags(FLAG_TRANSLUCENT_STATUS), and
  Window.setStatusBarColor() using the primary color from styles.md through AndroidJavaObject; all three are required
...
- Header sits above the list; 100 height applies to the list area only
